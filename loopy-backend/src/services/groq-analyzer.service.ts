...
export class GroqAnalyzerService {
  ...
  /**
   * Generate a beginner-friendly hint using the fastest available model (Llama 8B).
   */
  async generateHint(params: AIAnalysisParams): Promise<{ hint: string }> {
    ...
    const cascade = ['8b', 'scout', 'qwen'] // Prefer fast models for hints
    ...
    for (const modelKey of cascade) {
      ...
      try {
        ...
        const completion = await this.client.chat.completions.create({
          ...
          temperature: 0.2, // Lower temp for more focused hint
          max_tokens: 256, // Short hint
        ...
        logger.info('Groq hint generated', { 
          responseTime: Date.now() - startTime,
          model: model.label
        })
        
        return { hint: parsed.hint || 'Hãy đọc kỹ lại đề bài và xem bạn có bỏ lỡ phần nào không nhé.' }
      } catch (error: any) {
        lastError = error
        logger.warn(`Groq ${model.label} failed for hint, trying next`, { error: error.message })
      }
    }

    throw lastError || new Error('All Groq models exhausted for hint')
  }
