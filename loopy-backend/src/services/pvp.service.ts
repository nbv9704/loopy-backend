...
export const pvpService = {
  ...
  async createMatch(
    ...
    try {
      ...
      // Progress-based filtering: Only get questions the user has "unlocked"
      // based on their completed chapters.
      if (userId && config.language_id) {
        try {
          const { data: progressData } = await supabaseAdmin
            .from('user_progress')
            .select('lesson_id, lessons!inner(chapter_id, chapters!inner(order_index))')
            .eq('user_id', userId)
            .eq('status', 'completed')
            .eq('lessons.chapters.language_id', config.language_id)

          if (progressData && progressData.length > 0) {
            const maxOrderIndex = Math.max(...progressData.map((p: any) => p.lessons.chapters.order_index))
            const allowedChapterTags = Array.from({ length: maxOrderIndex }, (_, i) => `chapter:${i + 1}`)
            
            // Filter: questions that have an allowed chapter tag OR no chapter tags at all
            // We use overlaps for the array column
            query = query.or(`tags.overlaps.{${allowedChapterTags.join(',')}},tags.is.null`)
            logger.info(`Filtering PvP questions for user ${userId} up to chapter ${maxOrderIndex}`)
          } else {
            // Beginner with no progress: only show questions without chapter tags or marked basics
            query = query.or(`tags.is.null,tags.overlaps.{basics}`)
            logger.info(`Filtering PvP questions for new user ${userId} (basics only)`)
          }
        } catch (err) {
          logger.error('Error fetching progress for pvp filtering:', err)
          // Fallback: don't filter by progress if it fails
        }
      }
