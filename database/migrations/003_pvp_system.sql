...
BEGIN
  IF NEW.status = 'completed' AND OLD.status != 'completed' THEN
    ...
    SET 
      ...
      matches_won = matches_won + CASE WHEN p.user_id = NEW.winner_id THEN 1 ELSE 0 END,
      matches_lost = matches_lost + CASE WHEN p.user_id != NEW.winner_id THEN 1 ELSE 0 END,
      total_score = total_score + p.total_score,
      current_win_streak = CASE 
        WHEN p.user_id = NEW.winner_id THEN current_win_streak + 1 
        ELSE 0 
      END,
      longest_win_streak = GREATEST(
        longest_win_streak,
        CASE WHEN p.user_id = NEW.winner_id THEN current_win_streak + 1 ELSE current_win_streak END
      ),
      updated_at = NOW()
