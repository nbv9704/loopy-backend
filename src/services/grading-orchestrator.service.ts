...
export class GradingOrchestratorService {
  ...
  async gradeSubmission(params: GradeSubmissionParams): Promise<GradingResult> {
    ...
    if (!params.lessonTitle) {
      // No client context — try DB lookup
      const { data: exercise } = await supabaseAdmin
        .from('exercises')
        .select('*, lessons(title, description, insight, chapters(languages(id, name)))')
        .eq('id', params.exerciseId)
        .single()

      if (exercise) {
        exerciseTitle = exercise.title || exercise.instructions || 'Bài tập'
        exerciseQuestion = exercise.question || exercise.instructions || ''
      } else {
        const { data: lesson } = await supabaseAdmin
          .from('lessons')
          .select('id, lesson_id, title, description, insight, content')
          .eq('id', params.exerciseId)
          .single()

        if (lesson) {
          exerciseTitle = lesson.title || lesson.lesson_id || 'Bài tập'
          exerciseQuestion = lesson.description || lesson.content || ''
    ...
    if (gradingMethod === 'test' || gradingMethod === 'both') {
      ...
      if (tcError) {
        logger.error('Failed to fetch test cases', { error: tcError })
      }
      ...
      try {
        if (mappedTestCases.length > 0) {
          testResult = await this.testRunner.runTests(params.code, mappedTestCases, params.language)
