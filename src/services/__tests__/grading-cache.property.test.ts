...
describe('Property-Based Tests: Grading Cache Service', () => {
  ...
  describe('Property: Cache get/set consistency', () => {
    test('should retrieve the same value that was set (valid keys only)', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.string({ minLength: 5, maxLength: 50 }).filter(k => k !== '__proto__' && k !== 'constructor' && k !== 'prototype'),
    ...
    test('should ignore and return null for invalid prototype keys', async () => {
      await fc.assert(
        fc.asyncProperty(
          ...
          async (key, value) => {
            ...
            // Get should return null, not the value, because it was ignored
            const retrieved = await cacheService.get(key)
  ...
  describe('Property: Cache statistics', () => {
    test('hit rate should be between 0 and 1', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.array(fc.string({ minLength: 5, maxLength: 20 }).filter(k => k !== '__proto__' && k !== 'constructor' && k !== 'prototype'), { minLength: 1, maxLength: 10 }),
          async keys => {
            // Set some values
