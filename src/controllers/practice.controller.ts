...
export const listSets = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await practiceService.listSets({
      ...
      official: req.query.official as boolean | undefined,
...
export const createSet = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const isAdmin = await practiceService.checkIsAdmin(req.user!.id)
    const result = await practiceService.createSet(req.body, req.user!.id, isAdmin)
...
export const updateSet = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const isAdmin = await practiceService.checkIsAdmin(req.user!.id)
    const result = await practiceService.updateSet(req.params.setId, req.body, req.user!.id, isAdmin)
    res.json({ success: true, data: result })
  } catch (error) {
    next(error)
  }
...
export const deleteSet = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const result = await practiceService.deleteSet(req.params.setId, req.user!.id)
    res.json(result)
  } catch (error) {
    next(error)
  }
